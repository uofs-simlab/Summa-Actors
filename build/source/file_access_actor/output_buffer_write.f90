    ...
    do iStruct=1,size(structInfo)
      do iGRU=start_gru, max_gru
        do iHRU=1,size(gru_struc(iGRU)%hruInfo)
          ...
          case('attr')
            call writeParam(ncid,0,gru_struc(iGRU)%hruInfo(iHRU)%hru_ix,   &
          ...
          case('mpar')
            do iDOM=1,gru_struc(iGRU)%hruInfo(iHRU)%domCount
              call writeParam(ncid,iDOM,gru_struc(iGRU)%hruInfo(iHRU)%hru_ix,   &
                             summa_struct(1)%mparStruct%gru(iGRU)%hru(iHRU)%dom(iDOM), &
        ...
        select case(trim(structInfo(iStruct)%structName))
          case('bpar')
            call writeParam(ncid,0,iGRU,summa_struct(1)%bparStruct%gru(iGRU), &
...
subroutine writeData(isBvar, ncid,outputTimestep,maxLengthAll,output_step,&
  ...
  integer(i4b)                     :: iDOM              ! domain index
  integer(i4b)                     :: nDOMwrite         ! number of domain slices to write
    ...
    do iVar = 1,size(meta)
      ...
      do iDOM = 1, nDOMwrite
        if(meta(iVar)%varType==iLookVarType%scalarv) then
          call writeScalar(isBvar, hasDom, iDOM, ncid, outputTimeStep, output_step, &
                           minGRU, maxGRU, nHRUrun, iFreq, iVar, meta, stat,   &
                           map, err, cmessage)
        else ! non-scalar variables: regular data structures
          call writeVector(isBvar, hasDom, iDOM, ncid, outputTimeStep, maxLengthAll, output_step, minGRU, &
...
! **********************************************************************************************************
subroutine writeScalar(isBvar, hasDom, iDOM, ncid, outputTimestep, output_step, minGRU, maxGRU, &
    ...
    class is (gru_hru_dom_time_doubleVec)
      ...
      do iGRU = minGRU, maxGRU
        do iHRU = 1, gru_struc(iGRU)%hruCount
          ...
          if (iDOM > gru_struc(iGRU)%hruInfo(iHRU)%domCount) cycle
          if(.not.summa_struct(1)%finalizeStats%gru(iGRU)%hru(iHRU)%tim(output_step)%dat(iFreq)) cycle
          realVec(hruCounter, 1) = stat%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(map(iVar))%tim(output_step)%dat(iFreq)
        end do ! iHRU
      end do ! iGRU
...
! **********************************************************************************************************
subroutine writeVector(isBvar, hasDom, iDOM, ncid, outputTimestep, maxLengthAll, output_step, minGRU, maxGRU, &
  ...
  do iGRU = minGRU, maxGRU
    do iHRU=1,gru_struc(iGRU)%hruCount
      hruCounter = hruCounter + 1  ! will be iGRU if bvar
      if(.not.summa_struct(1)%finalizeStats%gru(iGRU)%hru(iHRU)%tim(output_step)%dat(iFreq)) cycle
      if(hasDom .and. iDOM > gru_struc(iGRU)%hruInfo(iHRU)%domCount) cycle

      ! get the model layers
      if(hasDom)then
        nSnow   = indx%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(iLookIndex%nSnow)%tim(output_step)%dat(1)
        nLake   = indx%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(iLookIndex%nLake)%tim(output_step)%dat(1)
        nSoil   = indx%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(iLookIndex%nSoil)%tim(output_step)%dat(1)
        nGlce   = indx%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(iLookIndex%nGlce)%tim(output_step)%dat(1)
        nLayers = indx%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(iLookIndex%nLayers)%tim(output_step)%dat(1)
      else
      ...
      select type (datt)
        class is (gru_hru_dom_time_doubleVec); realArray(hruCounter,1:datLength) = datt%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(iVar)%tim(output_step)%dat(1:datLength)
        class is (gru_hru_dom_time_intVec);     intArray(hruCounter,1:datLength) = datt%gru(iGRU)%hru(iHRU)%dom(iDOM)%var(iVar)%tim(output_step)%dat(1:datLength)
  ...
  if(hasDom)then
    select case(dataType)
      case(ixReal);    err = nf90_put_var(ncid%var(iFreq),meta(iVar)%ncVarID(iFreq),realArray(1:nSpace,1:maxLength),start=(/iDOM,minGRU,1,outputTimestep(iFreq)/),count=(/1,nSpace,maxLength,1/))
      case(ixInteger); err = nf90_put_var(ncid%var(iFreq),meta(iVar)%ncVarID(iFreq),intArray(1:nSpace,1:maxLength),start=(/iDOM,minGRU,1,outputTimestep(iFreq)/),count=(/1,nSpace,maxLength,1/))
    end select
  else
    select case(dataType)
